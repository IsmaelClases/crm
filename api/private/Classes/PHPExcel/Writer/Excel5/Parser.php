...
class PHPExcel_Writer_Excel5_Parser
{
	...
	function _convert($token)
	{
		...
		// match references like A1 or $A$2
		} elseif (preg_match('/^\$?([A-Ia-i]?[A-Za-z])\$?(\d+)$/',$token)) {
			return $this->_convertRef2d($token);

		// match external references like Sheet1!A1 or Sheet1:Sheet2!A1 or Sheet1!$A$2 or Sheet1:Sheet2!$A$2
		...
		// match ranges like A1:B2 or $A$2:$B$2
		} elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)\:(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)$/', $token)) {
			return $this->_convertRange2d($token);

		// match external ranges like Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or Sheet1!$A$2:$B$2 or Sheet1:Sheet2!$A$2:$B$2
	...
	{
		$class = 0; // formulas like Sheet1!$A$2:$A$2 in list type data validation need this class (0x3B)
	...
	function _match($token)
	{
		switch($token) {
			...
			case "<":
				...
				break;
			default:
				// if it's a reference A1 or $A$2 or $A1 or A$2
				...
				// If it's an external reference (Sheet1!A1 or Sheet1:Sheet2!A1 or Sheet1!$A$2 or Sheet1:Sheet2!$A$2)
				...
				// if it's a range A1:A2 or $A$2:$A$2
				elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+:(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/', $token) and
					   !preg_match("/[0-9]/",$this->_lookahead))
				{
					return $token;
				}
				// If it's an external range like Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or Sheet1!$A$2:$B$2 or Sheet1:Sheet2!$A$2:$B$2
	...
	function _fact()
	{
		...
		// if it's a range A1:B2 or $A$2:$B$2
		...
		// If it's an external range (Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or Sheet1!$A$2:$B$2 or Sheet1:Sheet2!$A$2:$B$2)
		...
		// If it's an external range ('Sheet1'!A1:B2 or 'Sheet1'!A1:B2 or 'Sheet1'!$A$2:$B$2 or 'Sheet1'!$A$2:$B$2)
