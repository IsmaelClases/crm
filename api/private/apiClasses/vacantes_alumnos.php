...
class VacantesAlumnos extends Conexion
{
  ...
  public function get()
  {
    $sql = $this->conexion->prepare("SELECT vac.*,
              (SELECT COUNT(sgi_vacantes_X_alumnos.id_alumno)
                  FROM sgi_vacantes_X_alumnos
                  INNER JOIN sgi_vacantes ON sgi_vacantes.id_vacante = sgi_vacantes_X_alumnos.id_vacante
                  WHERE sgi_vacantes_X_alumnos.id_vacante = vac.id_vacante
              ) AS alumnos_asignados,
              ent.entidad,
              unc.unidad_centro
          FROM sgi_vacantes vac
          INNER JOIN sgi_entidades ent ON vac.id_entidad = ent.id_entidad
          INNER JOIN sgi_unidades_centro unc ON vac.id_unidad_centro = unc.id_unidad_centro;
        ");
    $exito = $sql->execute();
